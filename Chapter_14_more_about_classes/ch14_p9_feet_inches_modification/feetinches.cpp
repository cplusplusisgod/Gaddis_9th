...
ostream&
operator<<(ostream& strm, const FeetInches& obj)
{
...
istream&
operator>>(istream& strm, FeetInches& obj)
{
...
FeetInches::operator<=(const FeetInches& obj)
{
  ...
  int obj_total = (obj.feet * 12) + obj.inches;
  int this_total = (this->feet * 12) + this->inches;

  if (this_total <= obj_total) {
    return true;
  } else {
    return false;
  }
...
FeetInches::operator!=(const FeetInches& obj)
{
  if (this->feet != obj.feet || this->inches != obj.inches)
    return true;
  else
    return false;
}
